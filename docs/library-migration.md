...
*capability* (layout, forms, feedback, data, ΓÇª) rather than by consumer.
...
1. **No domain types in primitive props.** A primitive takes
   ...
   or `@/generated/prisma`, it isn't a primitive any more ΓÇö move it into a
   composite folder.
...
|---|---|
| `chrome/` | Typographic + decorative atoms ΓÇö `Icon`, `IconTile`, `EyebrowLabel`, `MetaPill`, `Chip`, `Badge`, `StatusPill`, `Gradient` |
| `layout/` | Page-level scaffolding ΓÇö `PageSection`, `CardGrid`, `CalloutBanner`, `CtaPanel`, `Card`, `Panel`, `Section` |
| `content/` | Composite content blocks ΓÇö `FeatureCard`, `StatCard`, `EmptyState`, `StubPanel` |
| `controls/` | Interactive atoms ΓÇö `Button`, `IconButton`, `LinkButton` |
| `forms/` | Form primitives + `EntityForm` composite ΓÇö `Field`, `Fieldset`, `FormActions`, `Input`, `TextArea`, `Select`, `Checkbox`, `Switch`, `EntityForm` |
| `feedback/` | Modal family ΓÇö `Modal`, `Drawer`, `ConfirmDialog` |
| `data/` | Grid + table primitives + `EntityGrid` composite ΓÇö `DataTable`, `Toolbar`, `SearchInput`, `FilterChip`, `Pagination`, `EntityGrid` |
| `nav/` | Navigation atoms ΓÇö `Breadcrumb`, `AnchorNav`, `Tabs`, `Tooltip`, `Accordion` |
| `motion/` | Animation hooks/components ΓÇö `Reveal`, `useCountUp` |
| `theme/` | `ThemeProvider`, `useTheme` |

### Three composites worth knowing about

These are the structural ones ΓÇö they wrap many primitives into a
single declarative call.

**`<EntityGrid>`** ΓÇö schema-driven grid in two modes:
...
**`<EntityForm>`** ΓÇö schema-driven form. Field `kind`s: `text`, `email`,
...
**`<Accordion>`** ΓÇö uncontrolled-by-default expand/collapse list. Single
or multi-open. Uses the `.faq-*` CSS genre.

### Imports

```ts
// Top-level barrel (recommended ΓÇö most ergonomic)
...
(`EcosystemContent.tsx`) was the biggest single deletion (~ΓêÆ465 lines).
...
### Admin portal ΓÇö Phase 1: modals
...
`ResourcesAdmin`, `RefTableGrid`. Net deletion: ~ΓêÆ225 lines.

### Admin portal ΓÇö Phase 2: forms
...
### Admin portal ΓÇö Phase 4: grids
...
Net deletion: ~ΓêÆ334 lines.

### Admin portal ΓÇö Phase 5: editor forms

- `RefRowForm.tsx` migrated to `<EntityForm>` ΓÇö the 30 ref-table edit
  pages now use one schema-driven form. ΓêÆ71 lines.
...
- `ResourceEditForm.tsx` (874 lines) and `ProviderResourceEditForm.tsx`
  (940 lines) ΓÇö both migrated to library inputs in bulk via a Python regex
...
Phase 4c (`<EntityGrid>` client-mode) shipped, these are ready to migrate ΓÇö
see the recipe in ┬º3 below.
...
- `admin/ProviderVerifyForm.tsx` ΓÇö `<button className="btn btn-primary">` ΓåÆ `<Button intent="primary">`
- `admin/ResourceLifecyclePanel.tsx` ΓÇö three `<button>` ΓåÆ `<Button>`
- `admin/ReviewDecideForm.tsx` ΓÇö one submit `<button>` ΓåÆ `<Button>`
- `portal/EditResourceForm.tsx` ΓÇö two `<button>` ΓåÆ `<Button>`
- `portal/NewResourceForm.tsx` ΓÇö one `<button>` ΓåÆ `<Button>`
- `portal/PortalProfileForm.tsx` ΓÇö one save `<button>` ΓåÆ `<Button>`
- `admin/RowActionMenu.tsx` ΓÇö collateral revert; not edited by the migration
- `portals/FilteredDataTable.tsx` ΓÇö collateral revert; not edited
- `portals/header/PortalNotifications.tsx` ΓÇö collateral revert; not edited
- `portals/header/PortalSearch.tsx` ΓÇö collateral revert; not edited
...
- <button type="button" className="btn btn-primary" onClick={ΓÇª} disabled={busy}>
+ <Button intent="primary" onClick={ΓÇª} disabled={busy}>
    Label
- </button>
+ </Button>
```
...
- const columns: FilteredColumn<Row>[] = [ΓÇª];
+ const columns: EntityColumn<Row>[] = [ΓÇª];
...
- Public ΓÇö `/`, `/ecosystem`, `/governance`, `/registry`, `/docs`, `/contact`
- Auth ΓÇö `/login`, `/register`, `/auth/reset`, `/auth/verify`
- Admin ΓÇö `/admin`, `/admin/providers`, `/admin/resources`, `/admin/users`, `/admin/ref/jurisdiction` (or any ref table), `/admin/complaints/[id]`, `/admin/contacts/[id]`
- Provider ΓÇö `/provider`, `/provider/resources`, `/provider/submissions`, `/provider/complaints`
- Verifier / Sovereign ΓÇö stub pages, just check they render
...
intended source ΓÇö they are a workaround for the network-mounted Windows
...
(or the PowerShell / Git Bash equivalent). They are harmless ΓÇö TypeScript
ignores them ΓÇö but they aren't load-bearing.
...
cat src/components/admin/RefTableGrid.tsx   # 188 lines ΓÇö adapter to EntityGrid

# See a thin form adapter
cat src/components/admin/RefRowForm.tsx     # 115 lines ΓÇö adapter to EntityForm
