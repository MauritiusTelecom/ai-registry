-- Seed: Mauritius Mobile Operator packaged skill (ResourceType = skill).
-- Content derived from mauritius-mobile-operator SKILL.md / README:
--   ICTA-assigned prefix lookup for +230 mobile numbers (Emtel, my.t mobile / Cellplus, Chili / MTML);
--   deterministic dictionary lookup; MIT license; portability caveat for MNP.
--
-- Idempotent: safe to re-run. Expects Phase 1 reference data + provider slug `mauritius-telecom`
-- and jurisdiction `MU` (standard `npm run db:seed`). Does not modify existing catalogue rows.

-- ─── ResourceType: skill (normally already seeded; insert only if missing) ───
...
-- ─── Resource + ResourceSovereigntyBasis + SovereigntyEvidence + ResourceEndpoint
--     (+ optional ResourceLanguage en, ResourceSector telecom) ─────────────────
DO $$
DECLARE
  v_provider_id   text;
