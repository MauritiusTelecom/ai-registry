...
| `packages/ui-kit` (`@airegistry/ui-kit`) | Design tokens, headless React primitives, the `<PluginSlot>` system (planned) | Branding singletons, data fetching, anything portal-route-specific |
| `apps/portal` (`@airegistry/portal`) | Next.js routes (public + admin + provider + verifier + sovereign), components, branding singleton, theme cookie, public-origin helper, REST `/api/...` and MCP `/api/mcp` handlers (thin shims over core services) | Prisma schema, audit primitive, governance services, anything that should be reusable by third-party portals |
...
The plugin manifest types are in [`packages/sdk/src/plugin.ts`](packages/sdk/src/plugin.ts). The runtime loader lands at v1.0; manifest contributions are accepted now and will be wired up when the loader is merged.
