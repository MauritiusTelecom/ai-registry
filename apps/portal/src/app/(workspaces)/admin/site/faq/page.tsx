  ...
  return (
    <div className="p-content">
      <div className="p-page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
        <div>
          <h1 className="p-title">FAQ entries</h1>
          <p className="p-subtitle">
            Edit the questions shown in the public home-page FAQ section.
            Toggle <code>active</code> to hide an entry without losing edit
            history; reorder by editing each entry&rsquo;s sort order.
          </p>
        </div>
      ...
      {rows.length === 0 ? (
        ...
        >
          No FAQ entries yet. Run <code>pnpm db:seed</code> to populate the
          defaults, or <Link href="/admin/site/faq/new">add the first entry</Link>.
        </div>
      ) : (
          ...
          <table className="p-table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ textAlign: "left", color: "var(--text-3)", fontSize: 12 }}>
                ...
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>Question</th>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>Status</th>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>Updated</th>
