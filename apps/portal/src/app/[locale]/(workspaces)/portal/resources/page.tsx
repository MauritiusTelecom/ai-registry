      ...
      <section className="section" style={{ paddingTop: 24, paddingBottom: 24 }}>
        <div style={{ maxWidth: 960, margin: "0 auto", display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 20 }}>
          {LIFECYCLE_FILTERS.map((code) => {
            const label = code === "" ? t("filterAll") : code.replace(/_/g, " ");
            const href =
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: "var(--text-2)" }}>
                      {r.resourceTypeCode} · <span style={{ textTransform: "capitalize" }}>{r.lifecycleCode.replace(/_/g, " ")}</span>
                      {r.airId ? (
                        <>
                          {" "}
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {(r.lifecycleCode === "draft" || r.lifecycleCode === "needs_update") && (
                      <Link href={`/portal/resources/${r.id}`} className="btn btn-secondary">
                        {t("edit")}
                      </Link>
                    )}
                    {r.lifecycleCode === "listed" && r.slug ? (
