    ...
    <div className="p-content">
      ...
      {rows.length === 0 ? (
        <div style={{ padding: 28, borderRadius: 12, border: "1px dashed var(--border)", color: "var(--text-2)", fontSize: 14 }}>
          ...
          <Link href="/admin/site/how-it-works/new">{t("emptyLinkText")}</Link>.
        </div>
      ) : (
          ...
          <table className="p-table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ textAlign: "left", color: "var(--text-3)", fontSize: 12 }}>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>{t("colNumber")}</th>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>{t("colCode")}</th>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>{t("colTitle")}</th>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>{t("colHighlight")}</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "10px 12px", fontSize: 13 }}>{r.highlight ? t("yes") : "—"}</td>
