...
export async function generateMetadata() {
  return workspaceMetadata("sovereign.topology");
}
...
export default async function SovereignTopologyPage() {
  const t = await getTranslations("sovereign.topology");
  const cfg = getConfig();
    ...
    <div className="p-content">
      ...
      {providers.length === 0 ? (
        <p style={{ color: "var(--text-3)" }}>
          {t("noProviders", { jurisdiction: cfg.jurisdiction })}
        </p>
      ) : (
          ...
          {providers.map((p) => (
            ...
            >
              ...
              >
                ...
                >
                  {t("resourceCount", { count: p.resources.length })}
                </span>
              </header>
