...
export default async function SovereignRiskPage() {
  const i18n = await getTranslations("sovereign.risk");
  const cfg = getConfig();
  ...
  const tierCols: Column<Tier>[] = [
    ...
    { key: "total", label: i18n("columnTotal"), render: (row) => row.total, mono: true },
    { key: "listed", label: i18n("columnListed"), render: (row) => row.listed, mono: true },
    {
      key: "experimental",
      label: i18n("columnInFlight"),
      render: (row) => row.experimental,
      mono: true
    },
    { key: "isolated", label: i18n("columnIsolated"), render: (row) => row.isolated, mono: true }
  ];

  const resCols: Column<ResourceRow>[] = [
    {
      key: "title",
      label: i18n("columnResource"),
      render: (row) => (
        <Link
    ...
    { key: "provider", label: i18n("columnProvider"), render: (row) => row.provider },
    { key: "kind", label: i18n("columnType"), render: (row) => <span className="tag">{row.kind}</span> },
    {
      key: "lifecycle",
      label: i18n("columnLifecycle"),
  ...
  return (
    <div className="p-content">
      ...
      <h2 style={{ fontSize: 14, marginBottom: 10, color: "var(--text-2)" }}>{i18n("byTier")}</h2>
      <DataTable
        rows={tiers}
        columns={tierCols}
        emptyState={i18n("emptyTiers")}
        keyOf={(r) => r.code}
      />

      <h2 style={{ fontSize: 14, margin: "32px 0 10px", color: "var(--text-2)" }}>
        {i18n("highRiskResources")}
      </h2>
      <DataTable
        rows={highRows}
        columns={resCols}
        emptyState={i18n("emptyHighRisk", { jurisdiction: cfg.jurisdiction })}
        keyOf={(r) => r.id}
      />
    </div>
  );
