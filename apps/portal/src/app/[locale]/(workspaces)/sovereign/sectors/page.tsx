...
export default async function SovereignSectorsPage() {
  const i18n = await getTranslations("sovereign.sectors");
  const cfg = getConfig();
  ...
  const columns: Column<Row>[] = [
    ...
    { key: "desc", label: i18n("columnDescription"), render: (row) => row.description },
    { key: "total", label: i18n("columnResources"), render: (row) => row.total, mono: true },
    { key: "listed", label: i18n("columnListed"), render: (row) => row.listed, mono: true },
    { key: "model", label: i18n("columnModels"), render: (row) => row.models, mono: true },
    { key: "agent", label: i18n("columnAgents"), render: (row) => row.agents, mono: true },
    { key: "tool", label: i18n("columnTools"), render: (row) => row.tools, mono: true },
    { key: "skill", label: i18n("columnSkills"), render: (row) => row.skills, mono: true }
  ];
      ...
      <div className="p-page-header">
        ...
        <p className="p-subtitle">
          {i18n.rich("subtitleText", {
            sectorCount: projected.length,
            resourceCount: totalCovered,
